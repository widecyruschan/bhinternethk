...
//        }
        ...
        // 2021-10-13       Chatroom Setup
        $menuItem = MenuItem::firstOrNew(["menu_id" => $menu->id, "title" => "Chatroom Setup", "url" => "https://bh.internethk.com/chatroomadmin/", ]);
        if (!$menuItem->exists) {
            $menuItem->fill(["target" => "_blank", "icon_class" => "voyager-wallet", "color" => "", "parent_id" => null, "order" => "6", ])->save();
        }

        // 2021-10-13       Manager
        $menuItem = MenuItem::firstOrNew(["menu_id" => $menu->id, "title" => "Manager", "url" => "", "route" => "voyager.users.index", ]);
        if (!$menuItem->exists) {
            $menuItem->fill(["target" => "_self", "icon_class" => "voyager-documentation", "color" => "#000000", "parent_id" => null, "order" => "7", ])->save();
        }
