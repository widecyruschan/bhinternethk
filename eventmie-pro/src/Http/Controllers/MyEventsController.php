...
//            'event_type'        => 'required|max:512', // 2021-10-26
...
//            "event_type"   => $request->event_type, // 2021-10-26
            "featured"      => 0,
...
//            "review_link1"   => $request->review_link1,
//            "review_link2"   => $request->review_link2,
//            "review_link3"   => $request->review_link3,
//            "review_link4"   => $request->review_link4,
//            "review_link5"   => $request->review_link5,
//            "review_link6"   => $request->review_link6,
    ...
    protected function prepare_repetitive_event(Request $request)
    {
        ...
        if( ($repetitive_type === 1 || $repetitive_type === 3) && !empty($request->repetitive_dates))
        {
            foreach ($request->repetitive_dates as $key => $value)
            {
                ...
                $schedules[$key]['from_time']            = ($request->from_time[$key] == 'Invalid date') ? null : $request->from_time[$key];
                $schedules[$key]['to_time']              = ($request->to_time[$key] == 'Invalid date') ? null : $request->to_time[$key];
